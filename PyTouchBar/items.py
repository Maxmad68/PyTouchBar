...
class Popover(TouchBarItem):
    ...
    def reload(self):
        ...
        self.secondaryTouchBar.setDefaultItemIdentifiers_(
            list(map(lambda item: item.id, self.items))
        )
        ...
        if self.holdItems:
            ...
            self.holdTouchbar.setDefaultItemIdentifiers_(
                list(map(lambda item: item.id, self.holdItems))
            )
...
class Group(TouchBarItem):
    ...
    def makeItem(self):
        ...
        self.item = AppKit.NSGroupTouchBarItem.groupItemWithIdentifier_items_(
            self.id, instanciedItems
        )
        self.item.setCustomizationLabel_(self.customization_label)
        return self.item
...
class Label(TouchBarItem):
    ...
    def makeItem(self):
        ...
        self.label.setTextColor_(
            AppKit.NSColor.colorWithRed_green_blue_alpha_(*self.text_color_)
        )
    ...
    def textColorSetter(self, newValue):
        self.label.setTextColor_(
            AppKit.NSColor.colorWithRed_green_blue_alpha_(*newValue)
        )
...
class Button(TouchBarItem):
    ...
    def makeItem(self):
        ...
        self.button = AppKit.NSButton.buttonWithTitle_target_action_(
            title, self.actionManager, "buttonPressed:"
        )

        if self.image_:
            image = AppKit.NSImage.alloc().initByReferencingFile_(
                os.path.realpath(self.image_)
            )
        ...
        if self.color_:
            self.button.setBezelColor_(
                AppKit.NSColor.colorWithRed_green_blue_alpha_(*self.color_)
            )
...
class SegmentedControls(TouchBarItem):
    ...
    class Control(object):
        ...
        def __init__(self, **kwargs):
            ...
            if imageName:
                self.image = AppKit.NSImage.alloc().initByReferencingFile_(
                    os.path.realpath(imageName)
                )
            else:
                self.image = None
        ...
        def isSelected(self):
            if self.segmentedControl == None:
                ...
                return self.segmentedControl.NSSegmentedControl.isSelectedForSegment_(
                    self.index
                )
        ...
        def setSelected(self, status):
            if self.segmentedControl == None:
                ...
                return self.segmentedControl.NSSegmentedControl.setSelected_forSegment_(
                    status, self.index
                )
        ...
        def titleSetter(self, newValue):
            ...
            if self.segmentedControl:
                self.segmentedControl.NSSegmentedControl.setLabel_forSegment_(
                    newValue, self.index
                )
            else:
                print("i")
        ...
        def enabledSetter(self, newValue):
            ...
            if self.segmentedControl:
                self.segmentedControl.NSSegmentedControl.setEnabled_forSegment_(
                    newValue, self.index
                )
        ...
        def __repr__(self):
            return '<SegmentedControls.Item index = {idx}, title = "{title}">'.format(
                idx=self.index, title=self.title
            )
    ...
    def __init__(self, controls=["Segmented", "Controls"], **kwargs):
        ...
        self.customization_label = kwargs.get(
            "customization_label", "Segmented Controls"
        )
        self.customization_mode = kwargs.get("customization_mode", 0)
    ...
    def makeItem(self):
        ...
        segmentedControl = AppKit.NSSegmentedControl.segmentedControlWithLabels_trackingMode_target_action_(
            [], self.type, self.actionManager, "buttonPressed:"
        )
        ...
        for idx, control in enumerate(self.controls):
            if isinstance(control, SegmentedControls.Control):
                ...
                if control.image:
                    ...
                    segmentedControl.setImageScaling_forSegment_(
                        control.imageScaling, idx
                    )
    ...
    def makeItem(self):
        ...
        try:
            ...
            if self.type == ColorPicker.Type.color:
                self.item = AppKit.NSColorPickerTouchBarItem.colorPickerWithIdentifier_(
                    self.id
                )
            elif self.type == ColorPicker.Type.text:
                self.item = (
                    AppKit.NSColorPickerTouchBarItem.textColorPickerWithIdentifier_(
                        self.id
                    )
                )
            elif self.type == ColorPicker.Type.stroke:
                self.item = (
                    AppKit.NSColorPickerTouchBarItem.strokeColorPickerWithIdentifier_(
                        self.id
                    )
                )
                ...
                self.item = AppKit.NSColorPickerTouchBarItem.colorPickerWithIdentifier_buttonImage_(
                    self.id, image
                )
...
class Slider(TouchBarItem):
    ...
    def makeItem(self):
        ...
        if self.color:
            self.item.slider().setTrackFillColor_(
                AppKit.NSColor.colorWithRed_green_blue_alpha_(*self.color)
            )
