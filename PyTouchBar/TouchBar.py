...
class NSWindowControllerTBModified(AppKit.NSWindowController):
    ...
    def touchBar(self):
        ...
        tb.setCustomizationAllowedItemIdentifiers_(
            [
                item.id
                for item in touchbar_items_id.values()
                if item.customization_mode == 1
            ]
        )
        tb.setCustomizationRequiredItemIdentifiers_(
            [
                item.id
                for item in touchbar_items_id.values()
                if item.customization_mode == 2
            ]
        )
...
class TouchBarDelegate(Foundation.NSObject):
    ...
    def touchBar_makeItemForIdentifier_(self, bar, ident):
        ...
        if ident not in touchbar_items_id.keys():
            warnings.warn(
                "Requested ID that doesn't belong to any TouchbarItem", InternalWarning
            )
            return
...
def set_customization_identifier(identifier, menu=None):
    ...
    if menu is not None:
        AppKit.NSApplication.sharedApplication().setAutomaticCustomizeTouchBarMenuItemEnabled_(
            menu
        )
...
def customize(sender=None):
    ...
    """
    AppKit.NSApplication.sharedApplication().toggleTouchBarCustomizationPalette_(sender)
...
def notify(a):
    ...
    for window in windows:
        window_controllers.append(
            NSWindowControllerTBModified.alloc().initWithWindow_(window)
        )
